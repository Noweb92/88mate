/**
 * Generates supabase/seeds/eligible_postcodes.sql from the official
 * immi.homeaffairs.gov.au postcode tables.
 *
 ...
 *
 * The raw range strings below are transcribed VERBATIM from those pages
 * (tables 1-6 on each). Ranges are inclusive per the official note.
 * "All postcodes in X" entries are expanded using the standard delivery
 * postcode ranges (documented next to each constant).
 *
 * Run: node scripts/generate-eligible-postcodes.mjs
 */
...
const SOURCE_DATE = "2026-06-12"; // date the official pages were fetched

// "All postcodes in X are eligible" — expanded to the standard street
// delivery ranges (PO-box-only ranges excluded). Slight over-inclusion
// of unassigned codes within a range is harmless and mirrors how the
// official ranges already work.
const ALL_NT = "0800 to 0899";
const ALL_SA = "5000 to 5799";
const ALL_TAS = "7000 to 7799";
const ALL_ACT = "2600 to 2620, 2900 to 2920";
const ALL_NORFOLK = "2899";

// ---------------------------------------------------------------------------
// Subclass 417 — official tables
// ---------------------------------------------------------------------------

// Tables 1 + 2: Remote and Very Remote Australia (tourism & hospitality)
const T417_REMOTE = [
  // NSW
  ...
  // Table 2 (from 1 July 2022)
  "4406, 4416, 4498",
  "7215",
].join(", ");

// Table 3: Northern Australia (tourism & hospitality)
const T417_NORTHERN = [
  // QLD
...
// Table 4: Regional Australia (plant/animal, fishing, tree farming,
// mining, construction)
const T417_REGIONAL = [
  // NSW
...
// Table 6: Natural disaster declared areas
const T417_DISASTER = [
  // NSW
...
// ---------------------------------------------------------------------------
// Subclass 462 — official tables (remote/northern/bushfire/disaster tables
// are identical in content to 417's on the official pages, but industry
// mappings differ; regional table is the same list with different industries)
// ---------------------------------------------------------------------------

const T462_REMOTE = T417_REMOTE; // same postcodes (tables 1+2 match verbatim)
const T462_NORTHERN = T417_NORTHERN; // table 3 matches verbatim
const T462_REGIONAL = T417_REGIONAL; // table 4 matches verbatim
const T462_BUSHFIRE = T417_BUSHFIRE; // table 5 matches verbatim
const T462_DISASTER = T417_DISASTER; // table 6 matches verbatim

// ---------------------------------------------------------------------------
// Industry mapping per designated area, per visa (from "Approved
// industries and areas for specified work" on each page)
// ---------------------------------------------------------------------------

const AREAS = {
  "417": [
    { raw: T417_REGIONAL, industries: ["plant_animal_cultivation", "fishing_pearling", "tree_farming", "mining", "construction"] },
    { raw: T417_NORTHERN, industries: ["tourism_hospitality"] },
    { raw: T417_REMOTE, industries: ["tourism_hospitality"] },
    { raw: T417_BUSHFIRE, industries: ["bushfire_recovery"] },
    { raw: T417_DISASTER, industries: ["natural_disaster_recovery"] },
  ],
  "462": [
    { raw: T462_REGIONAL, industries: ["plant_animal_cultivation", "construction"] },
    { raw: T462_NORTHERN, industries: ["tourism_hospitality", "plant_animal_cultivation", "fishing_pearling", "tree_farming", "construction"] },
    { raw: T462_REMOTE, industries: ["tourism_hospitality"] },
    { raw: T462_BUSHFIRE, industries: ["bushfire_recovery"] },
    { raw: T462_DISASTER, industries: ["natural_disaster_recovery"] },
  ],
};
...
function expand(raw) {
  ...
  for (const part of raw.split(/[,;]/)) {
    const p = part.trim();
    if (!p) continue; // the official page contains a stray double comma
    const range = p.match(/^(\d{4})\s+to\s+(\d{4})$/);
    if (range) {
      ...
      throw new Error(`Unparseable token: "${p}"`);
    }
  }
  return out;
}

const rows = new Map(); // key: visa|postcode -> Set(industries)
for (const [visa, areas] of Object.entries(AREAS)) {
  for (const { raw, industries } of areas) {
    for (const pc of expand(raw)) {
      const key = `${visa}|${pc}`;
      if (!rows.has(key)) rows.set(key, new Set());
      for (const ind of industries) rows.get(key).add(ind);
    }
  }
}
...
const CHUNK = 500;
let sql = `-- =============================================================
-- 88Mate — OFFICIAL eligible postcodes (subclasses 417 & 462)
-- GENERATED FILE — do not edit by hand.
-- Source: immi.homeaffairs.gov.au specified-work pages, fetched ${SOURCE_DATE}.
-- Regenerate with: node scripts/generate-eligible-postcodes.mjs
-- =============================================================

delete from public.eligible_postcodes;

`;

for (let i = 0; i < sorted.length; i += CHUNK) {
  const chunk = sorted.slice(i, i + CHUNK);
  sql += "insert into public.eligible_postcodes (postcode, visa_type, industries, source_updated_at) values\n";
  sql += chunk
    .map(([key, inds]) => {
      const [visa, pc] = key.split("|");
      const arr = [...inds].sort().map((x) => `"${x}"`).join(",");
      return `  ('${pc}', '${visa}', '{${arr}}', '${SOURCE_DATE}')`;
    })
    .join(",\n");
  sql += "\non conflict (postcode, visa_type) do update set industries = excluded.industries, source_updated_at = excluded.source_updated_at;\n\n";
}

const outPath = join(ROOT, "supabase", "seeds", "eligible_postcodes.sql");
mkdirSync(dirname(outPath), { recursive: true });
writeFileSync(outPath, sql);

const by417 = sorted.filter(([k]) => k.startsWith("417")).length;
const by462 = sorted.filter(([k]) => k.startsWith("462")).length;
console.log(`Wrote ${outPath}`);
console.log(`417: ${by417} postcodes | 462: ${by462} postcodes | total rows: ${sorted.length}`);
